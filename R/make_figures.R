...
# Define the data directory as the source and results as the output
base_dir <- "./data"
out_dir  <- "./results"
...
comparison_map <- tibble::tribble(
  ...
  "CX1_vs_CTLFX",    "Concuss_1x_5_year_V12_424528",            "CX1",          "CTL-FX",       "CX1 vs CTL-FX (n=68,375 per arm)",
  "CX2_vs_CX1",      "Concuss_2x_5_year_V12_(1)_(1)_677193",    "CX2",          "CX1",          "CX2 vs CX1 (n=22,547 per arm)",
  "CX3_vs_CX2",      "Concuss_3x_5_year_V12_327389",            "CX3",          "CX2",          "CX3 vs CX2 (n=8,911 per arm)"
)

comparison_strip_labels <- c(
  "CX1 vs CTL-FX (n=68,375 per arm)" = "CX1 vs CTL-FX\n(n=68,375 per arm)",
  "CX2 vs CX1 (n=22,547 per arm)" = "CX2 vs CX1\n(n=22,547 per arm)",
  "CX3 vs CX2 (n=8,911 per arm)" = "CX3 vs CX2\n(n=8,911 per arm)"
)

comparison_axis_labels <- c(
  "CX1 vs CTL-FX (n=68,375 per arm)" = "CX1\nvs\nCTL-FX",
  "CX2 vs CX1 (n=22,547 per arm)" = "CX2\nvs\nCX1",
  "CX3 vs CX2 (n=8,911 per arm)" = "CX3\nvs\nCX2"
)
...
forest_outcome_map <- outcome_map %>%
  filter(outcome != "Opioid-related") %>%
  mutate(forest_y_pos = c(8, 7, 6, 5, 3, 2, 1))
...
figure2_caption <- paste(
  ...
  "Each panel shows one pairwise comparison. Filled circles indicate associations significant after",
  "Benjamini-Hochberg false-discovery-rate correction across all 24 tests (q < 0.05); open circles",
  "indicate q >= 0.05. RR = relative risk; CI = confidence interval; CTL-FX = non-concussion",
  "pediatric forearm-fracture controls."
)
...
results_df <- results_df %>%
  mutate(
    fdr_p = p.adjust(logrank_p, method = "BH"),
    fdr_significant_05 = fdr_p < 0.05
  )
# SAVE TO RESULTS
write_csv(results_df %>% mutate(across(where(is.factor), as.character)), file.path(out_dir, "results.csv"))

results_for_plot <- results_df %>%
  filter(outcome != "Opioid-related") %>%
  left_join(forest_outcome_map %>% select(outcome, forest_y_pos), by = "outcome") %>%
  mutate(
    outcome = factor(outcome, levels = forest_outcome_map %>% arrange(outcome_order) %>% pull(outcome)),
    significant = fdr_significant_05,
  ...
  mutate(y_pos = forest_y_pos) %>%
  ungroup()

guide_lines <- tibble(yintercept = c(1:3, 5:8))
separator_line <- tibble(yintercept = 4)
stripe_rows <- tibble(ymin = c(7.5, 5.5, 2.5), ymax = c(8.5, 6.5, 3.5))
label_rows <- tibble(
  outcome = factor(forest_outcome_map$outcome, levels = forest_outcome_map %>% arrange(outcome_order) %>% pull(outcome)),
  y_pos = forest_outcome_map$forest_y_pos
)
...
make_forest_panel <- function(panel_df) {
  ...
  ggplot(panel_df, aes(x = rr, y = y_pos)) +
    ...
    scale_x_log10(
      limits = c(0.6, 4.4),
      breaks = c(0.6, 0.75, 1, 1.5, 2, 3),
      labels = label_number(accuracy = 0.01, trim = TRUE)
    ) +
    scale_y_continuous(limits = c(0.4, 9.45), breaks = NULL, expand = expansion(mult = c(0, 0))) +
    ...
    theme(
      ...
      plot.margin = margin(t = 22, r = 22, b = 0, l = 10)
    )
}
...
label_panel <- ggplot(label_rows, aes(x = 1, y = y_pos)) +
  geom_hline(data = separator_line, aes(yintercept = yintercept), inherit.aes = FALSE, linewidth = 0.35, color = "#9aa0a6") +
  geom_text(
    data = label_rows %>% filter(y_pos >= 5),
    aes(label = outcome),
    hjust = 0,
    size = 5.1,
  ...
  geom_text(
    data = label_rows %>% filter(y_pos <= 3),
    aes(label = outcome),
    hjust = 0,
    size = 5.1,
    family = figure_family,
...
figure2 <- (
  label_panel +
    ...
    forest_panels[[2]] +
    forest_panels[[3]] +
    plot_layout(widths = c(2.25, 2.65, 2.65, 2.65))
) / x_axis_label +
  plot_layout(heights = c(24, 1.5)) +
  plot_annotation(
    title = "Five-year relative risks across stepwise matched comparisons",
    subtitle = "Filled circles indicate BH-FDR q < 0.05 across 24 tests; open circles indicate q >= 0.05",
    theme = theme(
...
# SAVE TO RESULTS
write_csv(power_df %>% select(comparison, outcome, n_per_arm, p_control, p_exposed_at_MCID, power, meets_80pct_power), file.path(out_dir, "power_table.csv"))
...
save_plot_all_formats(figure2, file.path(out_dir, "figure2_forest"), width_in, height_in, dpi = 300)
save_plot_all_formats(figure3, file.path(out_dir, "figure3_power"), width_in, height_in, dpi = 300)
...
writeLines(
  ...
  con = file.path(out_dir, "figure_captions.txt")
)
